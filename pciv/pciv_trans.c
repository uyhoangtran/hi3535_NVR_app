 ...
 ******************************************************************************
  ...
  2.Date        : 2010/02/12
    Author      : Hi3520MPP
    Modification: 1、将读写消息端口的打开改由调用者执行;
                  2、调整PCIV_TRANS_SENDER_S 结构体中本地Buffer和对端Buffer的字段定义
                  3、原来的PCIV_Trans_GetBuffer接口拆分为两个接口
******************************************************************************/
        ...
        HI_S32 s32Size, HI_S32 *pOffset)
{
    ...
    *pOffset = -1;
    ...
    if (pBuf->s32WritePos >= s32ReadPos)/* 写指针大于等于读指针 */
    {
        if ((pBuf->s32WritePos + s32Size) <= (pBuf->s32Length))/* 空闲空间足够 */
        {
            *pOffset = pBuf->s32WritePos;
            s32Ret = HI_SUCCESS;
        }
        else if (s32Size < s32ReadPos)/* 空闲空间不够，如果读指针大于size，则从头开始写 */
        {
            *pOffset = 0;
            s32Ret = HI_SUCCESS;
        }
    ...
    {
        if ((pBuf->s32WritePos + s32Size) < (s32ReadPos))/* 写的数据不会超过读指针*/
        {
            *pOffset = pBuf->s32WritePos;
            s32Ret = HI_SUCCESS;
        }
...
HI_S32 PCIV_Trans_InitSender(PCIV_TRANS_ATTR_S *pstAttr, HI_VOID **ppSender)
{
    ...
    s32Ret = pthread_create(&pstSender->pid, NULL, PCIV_Trans_SenderThread, pstSender);
...
HI_VOID * PCIV_Trans_SenderThread(HI_VOID *p)
{
    ...
    while (pstSender->bThreadStart)
    {
        ...
        /* 发送数据的消息序号和释放数据的消息序号，差距越大，说明对端接收数据越不及时 */
